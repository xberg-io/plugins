#!/usr/bin/env bash
# kreuzberg MCP launcher — ensures a working kreuzberg binary is available, then
...
# compiled binary. Rather than require users to install kreuzberg first, this
...
#
# The kreuzberg CLI crate is NOT published to crates.io, so `cargo install
# kreuzberg-cli` (registry form) does not work and is intentionally absent. The
# only cargo path that works is `cargo install --git`, which compiles from the
# repo — see the final guidance below.
#
# Note on npx/uvx: the kreuzberg npm and PyPI CLI packages are being rolled out
...
# The `@kreuzberg/*` npm packages and the importable pip package are language
# SDKs/bindings, NOT the CLI — they are not used here.
#
# Note on the download channel: kreuzberg publishes CLI assets named
# `kreuzberg-cli-<triple>.tar.gz` (`.zip` on Windows) with NO version in the
...
case "$(uname -s)" in
MINGW* | MSYS* | CYGWIN* | Windows_NT) BINARY_NAME="kreuzberg.exe" ;;
esac
...
if want brew && have brew; then
  ...
  if brew install xberg-io/tap/kreuzberg >&2; then
    ...
    log "brew install reported success but kreuzberg is not on PATH; falling through"
  else
...
try_download() {
  ...
  *) ext="tar.gz" ;;
  ...
  # CLI archives wrap their contents in a "kreuzberg-cli-<triple>/" directory
  # holding the binary (and, on musl, a bundled lib/ tree). Locate the binary.
  src_dir="$ex/kreuzberg-cli-${triple}"
  ...
  mv "$src_dir"/* "$BIN_DIR"/
  chmod +x "$BIN"
  log "installed kreuzberg to $BIN"
  return 0
}
