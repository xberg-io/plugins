...
archives, academic) using the local `kreuzberg` CLI. OCR is built in via
...
runs `kreuzberg mcp --transport stdio`. You get one MCP server named
`kreuzberg` with tools for single-file extraction, batch extraction, MIME
detection, and cache management. Prefer those MCP tools when available; fall
back to shelling out to the `kreuzberg` binary directly when the server is
unreachable.
...
- `kreuzberg/SKILL.md` — full Kreuzberg surface across CLI, Python, Node, Rust.
...
When configuration gets non-trivial, prefer a `kreuzberg.toml` over long flag
