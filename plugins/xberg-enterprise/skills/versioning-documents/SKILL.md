...
```json
{
  ...
  "diff": { "…": "xberg ExtractionDiff shape" },
  "computed_at": "2026-06-20T10:00:00Z"
}
```

The `diff` field is the full `xberg::diff::ExtractionDiff`. The OpenAPI
schema declares it opaque because the Rust type recurses; decode it against
xberg's published diff schema if you need a typed surface — do not
