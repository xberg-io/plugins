...
description: >-
  Managed Xberg document intelligence at api.xberg.io. Use when the
  ...
  usage tracking — instead of running the local xberg CLI. Covers
...
the local `xberg` CLI, with two extras: jobs are asynchronous (webhook
...
- Uses the `@xberg-io/cloud` (npm) or `xberg-cloud-sdk` (PyPI) SDKs.
...
1. The TypeScript SDK (`@xberg-io/cloud`) — ESM, tree-shakable, generated
   from the OpenAPI 3.1 spec.
2. The Python SDK (`xberg-cloud-sdk`) — sync + async, `from_sandbox()`
...
| Situation | Use |
|---|---|
| You already have the `xberg` CLI installed and the file is on disk | Local (`xberg` plugin) |
...
2. **Sandbox key** — no signup, 24-hour TTL, 50-page quota, rate-limited to
   ...
   `sandbox-keys` skill.
3. **Local `~/.xberg/cloud.toml`** — for shell sessions, put the key in:
...
then `~/.xberg/cloud.toml`, and emits a setup reminder if neither is
present.
...
`TimeoutError`, all extending `XbergCloudError` (Python) /
`XbergError` (TypeScript).
...
- TypeScript SDK: <https://www.npmjs.com/package/@xberg-io/cloud>
- Python SDK: <https://pypi.org/project/xberg-cloud-sdk/>
