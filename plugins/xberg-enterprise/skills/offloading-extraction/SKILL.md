...
description: Use when the user wants to extract a document via the cloud rather than the local xberg CLI. Covers POST /v1/extract — JSON vs multipart bodies, URL crawls, options block, webhook attachment, and the async response shape.
---
...
- File is on disk but the local `xberg` CLI is not installed.
...
const client = new XbergCloud({ apiKey: process.env.XBERG_API_KEY! });
...
from xberg_enterprise import XbergCloud

with XbergCloud(api_key=os.environ["XBERG_API_KEY"]) as client:
