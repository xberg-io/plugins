...
Complete API reference for the Xberg document extraction library in Rust.
...
// Create errors
let err = XbergError::parsing("invalid PDF");
let err = XbergError::ocr("Tesseract failed");
let err = XbergError::validation("config invalid");
let err = XbergError::unsupported_format("application/unknown");
let err = XbergError::missing_dependency("tesseract");

// With source
let source = std::io::Error::new(std::io::ErrorKind::NotFound, "file missing");
let err = XbergError::parsing_with_source("corrupt PDF", source);
```
...
match extract_file("doc.pdf", None, &config).await {
    ...
    Err(xberg::XbergError::Io(e)) => {
        println!("File error: {}", e);
    }
    Err(xberg::XbergError::UnsupportedFormat(fmt)) => {
...
All fallible operations return `Result<T>` where errors are `XbergError`.
...
This reference is for Xberg 4.x.
