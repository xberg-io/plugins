...
Xberg uses a hierarchical configuration system supporting multiple formats and auto-discovery mechanisms. This reference covers all available configuration options, field names across programming languages, and loading strategies.
...
When no configuration file is explicitly specified, Xberg searches for configuration files in the following order:

1. Current working directory: `xberg.toml`, `xberg.yaml`, `xberg.json`
...
config = ExtractionConfig.from_file("xberg.toml")
...
Xberg uses consistent naming conventions across different contexts:
